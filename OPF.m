function [Pgen,ENS, NETminCo, PNETu, PDGu, nENS, ef] = OPF(x, NETPavM, LD, FDmecst, D)
...
%   UNTIL THE MOMENT THE COSTS OF THE FEEDERS ARE NOT CONSIDERED
     ...
     DGco = [D.PVCo(ones(D.NDn,1),:) D.WCo(ones(D.NDn,1),:) D.STCo(ones(D.NDn,1),:)].*D.pckg(ones(D.NDn,1),1:3); 
     % DGco = [D.PVCo(ones(D.NDn,1),:) D.WCo(ones(D.NDn,1),:) D.EVCo(ones(D.NDn,1),:) D.STCo(ones(D.NDn,1),:)].*D.pckg(ones(D.NDn,1),:);     
     MS_DG_VGco = [MSco DGco D.VGCo(ones(D.NDn,1),:)]; % add virtual gen
