...
for i = 1:Del.SCn
    ...
    MSpow=Del.MSmu; % we can make this a control variable
    % MSpow = normrnd(Del.MSmu, Del.MSmu/10); % random power generated by the main sources
    MSPav = MSpow.*NETmecst(:,1).*x_el(:,1);
    %% The Weather Condition
%     ws = wblrnd(Wea.WWSscale(mnth),Wea.WWSshape(mnth));   % wind speed per node matrix [m/s]
%     s = betarnd(Wea.PVa(1) , Wea.PVb(1) ); % Beta distributed solar irradiation per node matrix [kW/m^2] (general)
      Text=normrnd(Wea.muText(mnth),Wea.stdText(mnth)); % external environmental conditions
%     if 7 >td || td > 20  
%         s=0; % no irradiance during night hours
%     end 
    ...
    %  EV
    %  EVPav=DGmdl.Power_EV(Del.NDn); % available power or demanded power from electric vehicles
    %  STPav = STPav.*NETmecst(:,5).*x_el(:,4); 
    %     EV (operating state (1)discharging, (-1)charging  and (0)disconnected)
    %     EVan = sum(sum(x(:,4)~=0));       % number of allocated ev
    %     EVopst = zeros(size(x(:,4)));
    %     EVOpaux = zeros(EVan,1);
    %     EVcsum = cumsum([0;Del.EVOpp(:,td)]);
    %     for j = 1:EVan
    %         EVOpaux(j) = find(EVcsum > rand(), 1, 'first') - 4;
    %         if EVOpaux(j) == -2
    %             EVOpaux(j) = 1;
    %         end
    %     end
    %     EVopst(x(:,4)~=0) = EVOpaux;
    %     EVPav = EVP(Del);
    %     %EVPp = EVPav.*x(:,4).*EVopst;
    %     EVPav = EVPav.*NETmecst(:,4).*x(:,4).*EVopst;
    %     nzEVPav = (EVPav < 0);
    %
    %     LDel = LDel - nzEVPav.*EVPav;    % EVs are in charging state
    %     %EVPp(EVPp < 0) = 0;
    %     EVPav(EVPav < 0) = 0;
    ...
    LDelbackup= (Dth.LumpedThLoadsMatrix*(NeededPowTh)); % suppose 100% efficent (if is an electric backup system is quite colose to 100%)
    ...
    % NETPavM = [MSPav PVPav WPav EVPav STPav].*[ones(Del.NDn,1) Del.pckg(ones(Del.NDn,1),:)]; %Matrix of electric power Available
